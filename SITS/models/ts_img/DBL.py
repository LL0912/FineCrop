...
@MODELS.register_module(name="DBL")
class DBL(nn.Module):
    def __init__(
        self,
        in_channel,
        ...
        self.inconv = ConvBlock(
            nkernels=[in_channel] + inconv, norm="group", pad_value=pad_value
        )
        ...
        for i in range(num_labelevel):
            if self.use_channel_fg:
                self.out_conv_dict.append(nn.Conv2d(hidden_size, num_classlevel[i]*channel_group, kernel_size=1))
            else:
                self.out_conv_dict.append(nn.Conv2d(hidden_size, num_classlevel[i], kernel_size=1))
        if self.use_channel_fg:
            for i in range(num_labelevel):
                self.classifier.append(nn.Conv2d(num_classlevel[i]*channel_group, num_classlevel[i], kernel_size=1))
    ...
    def temporal_interplate(self,x,target_t=12):
        #input: x: (B*C*H*W,T)
        x_reshaped = x.unsqueeze(1)  # [batch*C*H*W, 1, T]
        x_reshape_interp= F.interpolate(
            x_reshaped, 
            size=target_t, 
            mode='linear',  # 注意是线性插值！
            align_corners=False
        )
        branchT = x_reshape_interp.squeeze(1)# [batch*C*H*W,  T2]
        return branchT

    def forward(self, input, batch_positions=None,return_fea=False):
        pad_mask = (
        ...
        #将变化的T固定到不变的T
        x_reshaped = branchT.permute(0, 2, 3, 1).reshape(-1, T) #(B*C, T, H, W) -> (B*C*H*W,T)

        # 线性插值（需手动扩展维度模拟1D信号）
        branchT = self.temporal_interplate(x_reshaped).view(B*C, H, W, 12).permute(0, 3, 1, 2) #(B * C, T, H, W)

        
        branchT = self.TA(branchT) * branchT
        T2=12
        branchT = branchT.reshape(B, C, T2, H, W).permute(0, 2, 1, 3, 4) #(B,T2, C, H, W)
        ...
        x_reshaped = rearrange(branch1, 'b t c h w -> (b c h w) t')

        branch1 = self.temporal_interplate(x_reshaped).view(B, C, H, W, 12).permute(0, 4, 1, 2, 3) #(B, T, C, H, W)
 
        
        branch1 = branch1 + branchT
        branch1 = self.sa1.smart_forward(branch1).reshape(B * T2, 1, H, W) * branch1.reshape(B * T2, C, H, W)
        branch1 = branch1.reshape(B, T2, C, H, W)
        x_reshaped = rearrange(branch2, 'b t c h w -> (b c h w) t')
        branch2 = self.temporal_interplate(x_reshaped).view(B, C, H, W, 12).permute(0, 4, 1, 2, 3) #(B, T, C, H, W)
        out = torch.cat((branch1, branch2), dim=2)
        out = self.mid_conv.smart_forward(out)
        _, out = self.convlstmcan(out)
        out = out[0][1]
        ...
        for i in range(self.num_labelevel):
            if self.use_channel_fg:
                fea=self.out_conv_dict[i](out)
                out_g=self.classifier[i](fea)
                final_out[i] = out_g
                final_fea[i] = fea
            else:
                out_g = self.out_conv_dict[i](out)
                final_out[i]=out_g
...
#     model = DBL(
#         in_channel=10, num_classlevel=[9],
#         inconv=[32, 64],
#         n_levels=5,
#         n_channels=64,
#         hidden_size=88,
...
#         channel_group=2
#         ).cuda()
#     input = torch.randn(2,4, 10, 32, 32).cuda()
#     output,fea = model(input,return_fea=True)
#     print(output.shape)
#     print(fea.shape)
