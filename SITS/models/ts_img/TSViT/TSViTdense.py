...
@MODELS.register_module(name="tsvit")
class TSViT(nn.Module):
    ...
    def __init__(self,
        ...
        :param pool:
        :param scale_dim:
        :param time_source: optional:["doy","image"]
        """
        super().__init__()
        self.image_size = img_res
        self.patch_size = patch_size
        self.num_patches_1d = self.image_size//self.patch_size
        self.num_classes=num_classlevel[no_labellevel-1]
        ...
        assert self.pool in {'cls', 'mean'}, 'pool type must be either cls (cls token) or mean (mean pooling)'
        num_patches = self.num_patches_1d ** 2
        if self.time_source=="doy":
            patch_dim = num_channels* self.patch_size ** 2
        elif self.time_source=="image":
            patch_dim = (num_channels- 1) * self.patch_size ** 2  # -1 is set to exclude time feature
        self.to_patch_embedding = nn.Sequential(
            Rearrange('b t c (h p1) (w p2) -> (b h w) t (p1 p2 c)', p1=self.patch_size, p2=self.patch_size), ##rearrange将张量进行变形操作
            nn.Linear(patch_dim, self.dim),)
        ...
        if self.use_channel_fg:
            self.space_recover = nn.Sequential(
            nn.LayerNorm(self.dim),
            nn.Linear(self.dim, self.patch_size**2))
            ...
            self.channel_expand= nn.Sequential(
                nn.Conv2d(
                    ...
                    out_channels= self.num_classes*channel_group,
                    kernel_size=3,
                    padding=1,
                    stride=1,
                    padding_mode="reflect",
                ),
                nn.BatchNorm2d(self.num_classes*channel_group),
                nn.ReLU(),
            )

    def forward(self, x,doy,return_fea=False):
        B, T, C, H, W = x.shape
        if self.time_source=="doy":
            xt=doy[0, :].reshape(1, -1)
            xt=xt.repeat(B, 1)
        if self.time_source=="image":
            xt = x[:, :, -1, 0, 0] #[B,T]
            x = x[:, :, :-1] #[B,T,C-1,H,W]
            xt = (xt * 365.0001).to(torch.int64)
        xt = F.one_hot(xt, num_classes=366).to(torch.float32) #时间维度的特征取出来
        xt = xt.reshape(-1, 366)

        temporal_pos_embedding = self.to_temporal_embedding_input(xt).reshape(B, T, self.dim) ##时间编码
        x = self.to_patch_embedding(x) #打成patch后计算linear的特征 (b h w) ,t ,dim
        ...
        #加入分类token
        cls_temporal_tokens = repeat(self.temporal_token, '() N d -> b N d', b=B * self.num_patches_1d ** 2)#(bhw),k,dim
        x = torch.cat((cls_temporal_tokens, x), dim=1) #加入分类的token，输出(bhw),(t+k),dim

        #时间的transformer
        x = self.temporal_transformer(x)#(bhw),(t+k),dim
        x = x[:, :self.num_classes]#(bhw),(k),dim
        x = x.reshape(B, self.num_patches_1d**2, self.num_classes, self.dim).permute(0, 2, 1, 3).reshape(B*self.num_classes, self.num_patches_1d**2, self.dim) #(bk),(hw),dim

        #加入空间编码
        x += self.space_pos_embedding #(bk),(hw),dim
        x = self.dropout(x)
        ...
        if self.use_channel_fg:
            fea=self.space_recover(x.reshape(-1, self.dim))
            fea=fea.reshape(B, self.num_classes, H, W,) #[b,k,h,w]
            fea=self.channel_expand(fea) #[bkhw,dim]
        #分类头还原
        out = self.mlp_head(x.reshape(-1, self.dim)) #[bkhw,p**2]
        out = out.reshape(B, self.num_classes, self.num_patches_1d**2, self.patch_size**2).permute(0, 2, 3, 1)#[b,k,hw,p**2]->[b,hw,p**2,k]
        out = out.reshape(B, H, W, self.num_classes)# [B,H,W,K]
        out = out.permute(0, 3, 1, 2)# [B,K,H,W]
        if return_fea:
            if self.use_channel_fg:
                return out,fea
            else:
...
if __name__ == "__main__":
    ...
    x = torch.rand((3, 18, res, res, 10))
    doy = torch.linspace(1, 356, 18).to(torch.int64).reshape(1, -1)
    # model = TViT(model_config).cuda()
    # model = STViT(model_config)#.cuda()
    model = TSViT(use_channel_fg=True,
                 channel_group=2,)  # .cuda()
    ...
    out,fea = model(x,doy,return_fea=True)

    # torch.norm(cls, dim=2).shape
    print("Shape of out :", out.shape)  # [B, num_classes]
    print("Shape of fea :", fea.shape)  # [B, num_classes]